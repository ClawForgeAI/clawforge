...
export async function roleRoutes(app: FastifyInstance): Promise<void> {
   ...
   */
  app.get<{ Params: { orgId: string } }>("/api/v1/roles/:orgId", async (request, reply) => {
    ...
    const customRoles = await app.db.select().from(roles).where(eq(roles.orgId, orgId));
   ...
   */
  app.get<{ Params: { orgId: string } }>("/api/v1/roles/:orgId/permissions", async (request, reply) => {
    requirePermission(request, reply, "roles:read");
    if (reply.sent) return;
    const { orgId } = request.params;
    requireOrg(request, reply, orgId);
