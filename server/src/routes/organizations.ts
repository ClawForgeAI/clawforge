...
export async function organizationRoutes(app: FastifyInstance): Promise<void> {
   ...
   */
  app.get<{ Params: { orgId: string } }>("/api/v1/organizations/:orgId", async (request, reply) => {
   ...
   */
  app.put<{ Params: { orgId: string } }>("/api/v1/organizations/:orgId", async (request, reply) => {
    ...
    const [updated] = await app.db.update(organizations).set(updates).where(eq(organizations.id, orgId)).returning({
      id: organizations.id,
      name: organizations.name,
      ssoConfig: organizations.ssoConfig,
      createdAt: organizations.createdAt,
